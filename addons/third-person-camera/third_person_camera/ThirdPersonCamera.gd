...
##
@export_range(0.1, 10.0) var zoom_sensitivity := 1.0
...
# Cycle through zoom levels (4 -> 8 -> 12 -> 4...)
...
func set_zoom(new_distance: float) -> void:
	distance_from_pivot = clamp(new_distance, min_zoom_distance, max_zoom_distance)
	
	# Update the current_zoom_index when zooming with other methods
	# Find the closest zoom level
	var min_diff = INF
	var closest_index = current_zoom_index
	
	for i in range(zoom_levels.size()):
		var diff = abs(zoom_levels[i] - distance_from_pivot)
		if diff < min_diff:
			min_diff = diff
			closest_index = i
	
	current_zoom_index = closest_index
